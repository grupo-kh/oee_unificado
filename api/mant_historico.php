...
try {
    ...
    $limit = max(1, min(5000, (int)getParam('limit', '1000')));
    ...
    foreach ($machinesIdx as $codM => $m) {
        ...
        $machines[] = [
            'cod_maquina_mant'    => $m['cod_maquina_mant'],
            'desc_maquina'        => $m['desc_maquina'],
            'total_intervenciones' => $m['total_intervenciones'],
    ...
    foreach ($machines as $i => &$mch) {
        ...
        $mch['tasks'] = $newTasks;
        if ($truncado) {
            // Para no mostrar máquinas vacías por debajo del corte
