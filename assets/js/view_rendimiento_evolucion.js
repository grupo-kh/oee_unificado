/* Vista Rendimiento - Evolución (línea amarilla, filtros máquina/artículo) */

let chartRendEvo = null;
let _selCodMaquina  = '';
let _selCodArticulo = '';
...
function renderEvolucion(evo) {
    ...
    const promedio = evo.reduce((a, e) => a + parseFloat(e.rendimiento), 0) / evo.length;
    const maxV = Math.max(...evo.map(e => parseFloat(e.rendimiento)));
    ...
    const options = {
        ...
        series: [{
            name: 'Rendimiento',
            data: evo.map(d => ({ x: d.fecha, y: parseFloat(d.rendimiento), M_min: d.M_min, PPERF_min: d.PPERF_min }))
        }],
        xaxis: {
            type: 'datetime',
            labels: {
        ...
        yaxis: {
            min: 0, max: Math.max(120, Math.ceil(maxV / 10) * 10),
            labels: {
    ...
    if (chartRendEvo) chartRendEvo.destroy();
    chartRendEvo = new ApexCharts(el, options);
    chartRendEvo.render();
}
...
async function cargarVista() {
    ...
    try {
        ...
        const d = await apiFetch('rendimiento_evolucion.php', params);
