...
function renderGauge(value, label) {
    ...
    const options = {
        ...
        labels: [label || 'Rendimiento'],
        stroke: { lineCap: 'round' }
    };
    if (gaugeRend) gaugeRend.destroy();
    gaugeRend = new ApexCharts($('#gauge-rend'), options);
    gaugeRend.render();
}
...
function renderSecciones(secciones) {
    ...
    const data = _seccionesData.map(s => ({
        x: s.seccion, y: parseFloat(s.rendimiento),
        maquinas: s.maquinas, M_min: s.M_min, PPERF_min: s.PPERF_min
    }));
    const maxV = Math.max(...data.map(d => d.y), 100);

    // Stroke por barra: borde grueso oscuro en la sección activa.
    ...
    const options = {
        ...
        series: [{ name: 'Rendimiento', data: data.map(d => d.y) }],
        xaxis: {
            ...
            custom: ({dataPointIndex}) => {
                ...
                return `
                    <div style="padding:10px 12px;background:#1a2d4a;color:#fff;font-family:Arial;font-size:12px;min-width:220px">
                        ...
                        <div style="display:flex;justify-content:space-between;gap:12px"><span>PPERF</span><span>${fmt.format(r.PPERF_min)} min</span></div>
                        <div style="border-top:1px solid #3a5576;margin-top:6px;padding-top:6px;display:flex;justify-content:space-between;gap:12px;font-weight:700;font-size:13px">
                            <span>Rendimiento</span><span>${r.y.toFixed(1)}%</span>
                        </div>
    ...
    if (chartRendSecc) chartRendSecc.destroy();
    chartRendSecc = new ApexCharts($('#chart-secciones'), options);
    chartRendSecc.render();
}

function tooltipDetalleHTML(label, codigo, M_min, PPERF_min, y) {
    const fmt = new Intl.NumberFormat('es-ES');
    return `
        <div style="padding:10px 12px;background:#1a2d4a;color:#fff;font-family:Arial;font-size:12px;min-width:240px">
            ...
            <div style="display:flex;justify-content:space-between;gap:12px"><span>PPERF</span><span>${fmt.format(PPERF_min)} min</span></div>
            <div style="border-top:1px solid #3a5576;margin-top:6px;padding-top:6px;display:flex;justify-content:space-between;gap:12px;font-weight:700;font-size:13px">
                <span>Rendimiento</span><span>${parseFloat(y).toFixed(1)}%</span>
            </div>
...
function renderDrillDownArticulos(articulos) {
    ...
    const data = articulos.map(a => ({
        ...
        y: parseFloat(a.rendimiento),
        cod_articulo: a.cod_articulo,
        M_min: a.M_min, PPERF_min: a.PPERF_min
    }));
    const maxV = Math.max(...data.map(d => d.y), 100);
    ...
    const options = {
        ...
        tooltip: {
            custom: ({dataPointIndex}) => {
                const r = data[dataPointIndex];
                return tooltipDetalleHTML(r.label, r.x, r.M_min, r.PPERF_min, r.y);
            }
        },
        annotations: {
...
function renderDrillDownMaquinas(maquinas) {
    ...
    const data = maquinas.map(m => ({
        ...
        y: parseFloat(m.rendimiento),
        M_min: m.M_min, PPERF_min: m.PPERF_min
    }));
    const maxV = Math.max(...data.map(d => d.y), 100);
    const height = Math.max(220, Math.min(1400, 28 * data.length + 90));
    const options = {
        ...
        tooltip: {
            custom: ({dataPointIndex}) => {
                const r = data[dataPointIndex];
                return tooltipDetalleHTML(r.x, r.cod_maquina, r.M_min, r.PPERF_min, r.y);
            }
        },
        annotations: {
            xaxis: [{ x: 75, borderColor: '#10b981', strokeDashArray: 6 },
...
async function cargarDrillDown() {
    ...
    try {
        ...
        const d = await apiFetch('rendimiento_seccion_detalle.php', params);
...
function toggleDrillDown(seccion) {
    ...
    // Re-render del gráfico de secciones para refrescar el resalte.
...
async function cargarVista() {
    ...
    try {
        ...
        const d = await apiFetch('rendimiento_global.php', params);
        ...
        const labelGauge = scopeBits.length ? scopeBits.join(' · ') : 'Rendimiento';
        renderGauge(parseFloat(d.global.rendimiento), labelGauge);
